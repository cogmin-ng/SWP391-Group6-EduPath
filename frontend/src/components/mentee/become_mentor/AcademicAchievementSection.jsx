    ...
    <section className="rounded-3xl border border-slate-100 bg-white p-6 sm:p-8 shadow-sm mb-6 animate-fadeIn">
      ...
      <div className="space-y-3">
        {achievements.map((row, index) => {
          // Lọc ra các môn học chưa được chọn ở các hàng khác (để không bị trùng)
          const filteredOptions = subjects.filter(
            (s) =>
              !achievements.some(
                (a, i) => i !== index && a.subjectId === s.id
              )
          );
            ...
            <div key={index} className="flex items-center gap-3">
              ...
              <div className="relative flex-1">
                <select
                  ...
                  className={`w-full appearance-none ${inputCls} pr-10 cursor-pointer ${
                    subjects.length === 0 ? "opacity-70 cursor-not-allowed bg-slate-50" : ""
                  }`}
                >
                  <option value="">
                    {subjects.length === 0
                      ? "Vui lòng thêm môn học muốn mentor trước"
                      : "Chọn môn học"}
                  </option>
              ...
              <input
                ...
                className={`w-20 sm:w-24 text-center ${inputCls} ${
                  !row.subjectId ? "opacity-70 cursor-not-allowed bg-slate-50" : ""
                }`}
              />
